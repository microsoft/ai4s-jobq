...
from ai4s.jobq import MOLDYN_SUBSCRIPTION_ID, JobQ, __version__
...
@click.option(
    "--subscription",
    envvar="JOBQ_SUBSCRIPTION",
    default=MOLDYN_SUBSCRIPTION_ID,
    show_default=True,
    help="The subscription ID for the queue's storage account",
)
@click.option(
    "--conn-str",
...
@click.argument(
    "workspace_id",
    type=str,
    metavar="LOG_ANALYTICS_WORKSPACE_ID",
    envvar="JOBQ_LA_WORKSPACE_ID",
    required=True,
)
@click.option("--debug", is_flag=True, help="Enable debug logging.")
@click.pass_context
async def track(ctx: click.Context, workspace_id, debug) -> None:
    """
    ...
    if not _jobq_track_requirements_are_available():
        LOG.error(
            "JobQ Track requires packages you don't have installed (dash, dash_table, pandas, azure-monitor-query). "
    ...
    run_with_default_queue(f"{queue!s}/{queue.name}", debug=debug)
